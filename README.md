...
**Files.** Open `.txt`, `.scrb`, `.rtf`, `.md`, `.log`, `.csv`, `.json`, `.xml`, `.yaml`, `.yml`, `.ini`, `.cfg`. Save as `.txt`, `.scrb`, `.rtf`. Drag and drop from Explorer, including multi-file drops. RTF import and export round-trips bold, italic, fonts, sizes, lists, checklists, headers, quotes, links, and sub/superscript through Word and WordPad.
...
32 Dart files, roughly 13,200 lines, covered by 533 tests. See [ARCHITECTURE.md](ARCHITECTURE.md) for the module map, the load-bearing invariants, and save-path routing.
...
- **Recovery of a plaintext file you later encrypted.** Encrypting an existing `.txt` or `.rtf` writes the `.scrb` and then deletes the original. Deleting a file unlinks it, it does not overwrite the bytes, so the plaintext may remain recoverable from free space, a backup, a filesystem snapshot, or a synced folder. For notes that must never exist in plaintext, create the note encrypted rather than converting one.
